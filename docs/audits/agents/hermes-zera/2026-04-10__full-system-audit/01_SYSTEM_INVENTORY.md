...
- Проиндексировано 259 релевантных файлов по зонам `configs/personas`, `configs/tooling`, `configs/skills`, `scripts`, `Zera`, `logs`, `outputs/reliability`, `.agents/*`.
...
| `runtime_consumed_secondary` | `configs/orchestrator/router.yaml`, `configs/orchestrator/completion_gates.yaml`, `configs/orchestrator/user_profile.json`, `.agents/config/workflow_sets.active.json`, `.agents/runtime/*`, `.agents/memory/*`, `~/.hermes/profiles/antigravity/config.yaml`, `~/.gemini/antigravity/mcp_config.json`, `~/.hermes/profiles/zera/cron/*.json` |
| `derived/generated` | `docs/ki/benchmark_latest.*`, `docs/ki/benchmark_history.json`, `docs/ki/benchmark_anomalies.json`, `outputs/reliability/**`, `.agents/skills/.active_set_manifest.json`, `configs/orchestrator/catalog.json` |
...
- `.agents/workflows/multi-agent-routing.md` referenced in repo instructions and workflow indexes, but absent.
- `.agents/workflows/*` cataloged in `.agents/config/workflow_sets.active.json` and `configs/tooling/workflow_sets_catalog.json`, but directory is effectively empty.
- Zera skill set declared in `configs/skills/ZERA_ACTIVE_SKILLS.md`, but `.agents/skills/.active_set_manifest.json` publishes none of `zera-core`, `zera-strategist`, `zera-researcher`, `zera-rhythm-coach`, `zera-muse`, `zera-style-curator`.
...
2. Workflow catalog drift — `.agents/workflows/*` is empty; `workflow_sets_catalog.json` and `workflow_sets.active.json` reference non-existent files.
3. Skill publication drift — `configs/skills/ZERA_ACTIVE_SKILLS.md` declares 6 Zera skills; `.agents/skills/.active_set_manifest.json` publishes none of them.
...
### Dead References
- `.agents/workflows/multi-agent-routing.md` — referenced in AGENTS.md and workflow indexes but does not exist.
- `.agents/workflows/*` — entire directory effectively empty; all workflow references dangling.
...
- `.agents/skills/gsap-animation.md`, `gsap-performance-guardrails.md`, etc. — referenced in motion routing but not published.
