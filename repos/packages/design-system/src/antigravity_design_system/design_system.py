...
class DesignSystemGenerator:
    ...
    def generate(self, query: str, project_name: str = None) -> dict:
        ...
        # Fast Path Caching
        cache_file = DATA_DIR / "UI_TOKENS_CACHE.json"
        if cache_file.exists():
            try:
                with open(cache_file, 'r', encoding='utf-8') as f:
                    cache = json.load(f)
                    presets = cache.get("presets", {})
                    query_lower = query.lower()
                    for preset_name, data in presets.items():
                        if preset_name in query_lower:
                            return self._format_cached_result(data, project_name or query.upper())
    ...
    def _format_cached_result(self, data: dict, project_name: str) -> dict:
        """Format a cached preset into the standard design system structure."""
        return {
            "project_name": project_name,
            "category": "Cached Preset",
            "pattern": {
                "name": "Standard Landing",
                "sections": "Hero > Features > CTA",
                "cta_placement": "Header + Hero",
                "color_strategy": "Vibrant CTA",
                "conversion": "Focus on clarity"
            },
            "style": {
                "name": data.get("style", "Minimalism"),
                "type": "Preset",
                "effects": ", ".join(data.get("magic_ui", [])),
                "keywords": "fast-path, verified, premium",
            ...
            "colors": {
                ...
                "secondary": data.get("secondary", "#3B82F6"),
                "cta": data.get("primary", "#F97316"),
                "background": data.get("background", "#F8FAFC"),
                "text": data.get("text", "#1E293B"),
                "notes": "Fast-path preset"
            },
            "typography": {
            ...
            "key_effects": ", ".join(data.get("magic_ui", [])),
            "anti_patterns": "Default browser components",
            "decision_rules": {},
            "severity": "LOW"
        }
...
def format_ascii_box(design_system: dict) -> str:
    ...
    # Guardrails section
    lines.append("|  STITCH / LLM GENERATION GUARDRAILS:".ljust(BOX_WIDTH) + "|")
    guardrail_items = [
        "1. NO arbitrary hex colors. Use CSS variables: var(--ag-primary) etc.",
        "2. NO inline custom components. Import from @/components/ui/* (shadcn).",
        "3. NO local useState for API data. Use Props (e.g., onSubmit(data))."
    ]
...
def format_markdown(design_system: dict) -> str:
    ...
    # Guardrails section
    lines.append("### Stitch / LLM Generation Guardrails")
    lines.append("- **1. NO arbitrary hex colors.** Use CSS variables: `var(--ag-primary)` etc.")
    lines.append("- **2. NO inline custom components.** Import from `@/components/ui/*` (shadcn).")
    lines.append("- **3. NO local `useState` for API data.** Use Props (e.g., `onSubmit(data)`).")
    lines.append("")
...
def format_master_md(design_system: dict) -> str:
    ...
    lines.append("---")
    lines.append("")
    lines.append("## STITCH / MCP Generation Guardrails")
    lines.append("")
    lines.append("If this file is being used by Stitch or an Agent to generate code, these rules are ABSOLUTE:")
    lines.append("")
    lines.append("1. **Token Drift Prevention**: DO NOT use arbitrary hex colors or `text-blue-500` style utility classes randomly. You MUST use the assigned CSS variables (`var(--ag-primary)`, `var(--ag-secondary)`) mapped via Tailwind configuration.")
    lines.append("2. **Component Reuse**: DO NOT write custom UI primitives from scratch. Standard components (Button, Input, Card) MUST be imported from `@/components/ui/*` (shadcn-compatible structure).")
    lines.append("3. **Dead UI Prevention**: Form actions or interactive buttons MUST NOT contain hardcoded API logic. Rely on passed props like `onSubmit(data)` to keep the component cleanly decoupled from global state.")
    lines.append("")
